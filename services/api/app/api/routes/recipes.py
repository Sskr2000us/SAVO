...
def _recipe_schema() -> dict[str, Any]:
    # Centralized in prompt-pack for unified prompting + schema reuse.
    # Matches the Flutter `Recipe` model in apps/mobile/lib/models/planning.dart.
    return get_schema("RECIPE_SCHEMA_V1")
...
async def generate_detailed_recipe_collection(
    ...
    task = get_task("generate_recipe_collection_markdown")
    schema = get_schema(task.get("output_schema_ref") or "RECIPE_COLLECTION_MARKDOWN_SCHEMA")
    ...
    client = get_reasoning_client()
    try:
        result = await generate_json_with_retries(
            client=client,
            messages=[
            ...
            schema=schema,
            max_attempts=2,
            repair_hint="markdown recipe collection",
        )
